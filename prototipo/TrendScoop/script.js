...
// Adicione esta parte do código para lidar com a personalização
window.addEventListener('load', async () => {
  // Verifica se um país personalizado foi salvo
  const selectedCountry = localStorage.getItem('selectedCountry');

  if (selectedCountry) {
      // Use o país personalizado para carregar notícias
      await fetchNews(selectedCountry, 5);
  } else {
      // Caso contrário, carregue notícias dos Estados Unidos (ou país padrão)
      await fetchNews('us', 5);
  }

  // Atualiza a lista suspensa com a seleção atual do usuário (ou padrão)
  const countrySelect = document.getElementById('country-select');
  countrySelect.value = selectedCountry || 'us';

  // Adicione um ouvinte de evento para o botão "Salvar"
  const saveButton = document.getElementById('save-button');
  saveButton.addEventListener('click', () => {
      const selectedCountry = countrySelect.value;

      // Salve a seleção do usuário no Local Storage
